...
export default function Page() {
  ...
  const onSubmit: SubmitHandler<verificationCodeInputs> = async (data) => {
    const [, error] = await catchError(verfiyPassword(data.code));
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <p className="mt-4 text-right text-sm text-black">
              Did not receive a code ?<span className="text-main-color">Verfiy</span>
            </p>
            {/* Login Button */}
            <Button type="submit" className="w-full bg-blue-600 text-white hover:bg-blue-700 rounded-3xl mt-4">
              Verfiy
            </Button>
          </form>
        </Form>
